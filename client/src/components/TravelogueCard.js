...
const TravelogueCard = ({ travelogue, onBookmarkSave, onBookmarkUnsave }) => {
  ...
  const publishedDate = new Date(travelogue.created_at).toLocaleDateString(
    "en-US",
  ...
  const renderedBookmarkButton = user?.saved_posts.find(
    (post) => post.travelogue_id === travelogue.id
  ) ? (
        ...
        <Box sx={{ width: "100%", minWidth: "0", padding: "1rem" }}>
          <Box sx={{ display: "flex", flexDirection: "row" }}>
            <Avatar
              ...
              alt={travelogue?.user?.username}
              src={travelogue?.user?.avatar_url}
            />
            <Typography
              gutterBottom
              variant="body2"
          ...
          >
            ...
            <Box sx={{ display: "flex" }}>
              <Button size="small">{renderedBookmarkButton}</Button>
            </Box>
          </Box>
        </Box>
        <Box>
